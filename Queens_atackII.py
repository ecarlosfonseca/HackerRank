def queensAtackII(n, k, r_q, c_q, obstacles):
    ...
    n_houses_no_obs = n - r_q + r_q - 1 + n - c_q + c_q - 1 + min((n-c_q), (n-r_q)) + min((r_q-1), (c_q-1)) + \
                      min((n-c_q), (r_q-1)) + min((n-r_q), (c_q-1))

    print(n_houses_no_obs)
    ...
    for v in obstacles:
        ...
        if v[0] - r_q != 0 and v[1] - c_q == 0:
            if v[0] > r_q and v[0] - r_q < N:
                n_houses_no_obs -= (n-v[0]+1)
                N = v[0] - r_q-1

                print(v, '1retirei', (n-v[0]+1))

            elif v[0] < r_q and r_q - v[0] < S:
                n_houses_no_obs -= v[0]
                S = r_q - v[0] - 1


                print(v, '2retirei', v[0])

        elif v[0] - r_q == 0 and v[1] - c_q != 0:
            if v[1] > c_q and v[1] - c_q < E:
                n_houses_no_obs -= (n-v[1]+1)
                E = v[1] - c_q-1

                print(v, '3retirei', (n-v[1]+1))

            elif v[1] < c_q and c_q-v[1] < O:
                n_houses_no_obs -= v[1]
                O = c_q-v[1]-1

                print(v, '4retirei', v[1])

        elif abs(v[0] - r_q) == abs(v[1] - c_q):
            if v[0] > r_q and v[1] > c_q and abs(v[0] - r_q) < NE:
                n_houses_no_obs -= min(n-v[0], n-v[1]) + 1 - (min((n-c_q), (n-r_q)) - NE)
                NE = NE - (min(n-v[0], n-v[1]) + 1 - (min((n-c_q), (n-r_q)) - NE))

            elif v[0] > r_q and v[1] < c_q and abs(v[0] - r_q) < NO:
                n_houses_no_obs -= (min(n-v[0], v[1]))+1
                NO = abs(v[0] - r_q)

                print(v, '6retirei', (min(n-v[0], v[1]+1)))

            elif v[0] < r_q and v[1] < c_q and abs(v[0] - r_q) < SO:
                n_houses_no_obs -= min(v[0], v[1])
                SO = SO - min(v[0], v[1])

                print(v, '7retirei', min(v[0], v[1]))

            if v[0] < r_q and v[1] > c_q and abs(v[0] - r_q) < SE:
                n_houses_no_obs -= min(v[0], n-v[1]) + 1
                SE = SE - min(n-v[0], n-v[1])

                print(v, '8retirei', min(v[0], n-v[1])+1)
...
if __name__ == '__main__':
    ...
    queensAtackII(size3, n_obs3, l_q3, c_q3, obs3)
